...
MAX_TRADE_SIZE = 45.0     # Max USD spent per arbitrage trade ($22.50 per side)
MIN_EDGE = 0.025          # 2.5% minimum gross edge
EST_GAS_FEE_USD = 0.01    # Estimated Polygon Gas Fee ($0.01)
CRYPTO_TAKER_FEE_RATE = 0.072  # Polymarket Crypto Taker Fee (7.2%)
...
# Direct log output to sys.stdout so Railway doesn't flag INFO logs as errors
logging.basicConfig(
    level=logging.INFO,
...
session.headers.update({"User-Agent": "PolymarketCryptoWSBot/6.2"})
...
def calculate_vwap_and_slippage(asks, target_usd):
    ...
    slippage_pct = ((vwap_price - best_ask) / best_ask) * 100 if best_ask > 0 else 0.0
...
def save_trades_to_csv():
    ...
    try:
        ...
        logger.info(f"Successfully exported {len(trades_history)} trades to {CSV_FILENAME}")
...
def evaluate_arbitrage(market_id):
    ...
    target_spend_per_side = MAX_TRADE_SIZE / 2.0
    ...
    if gross_edge >= MIN_EDGE:
        ...
        trade_cost = (executable_shares * up_vwap) + (executable_shares * down_vwap)

        up_fee = calculate_polymarket_taker_fee(executable_shares, up_vwap)
        down_fee = calculate_polymarket_taker_fee(executable_shares, down_vwap)
        ...
        print(f"   UP  : Best ${up_best:.3f} | VWAP ${up_vwap:.3f} | Slippage: {up_slippage:.2f}%", flush=True)
        print(f"   DOWN: Best ${down_best:.3f} | VWAP ${down_vwap:.3f} | Slippage: {down_slippage:.2f}%", flush=True)
        print(f"   Spent: ${trade_cost:.2f} | Taker Fee: ${total_taker_fee:.4f} | Est Gas: ${EST_GAS_FEE_USD:.2f}", flush=True)
        print(f"   Payout: ${guaranteed_payout:.2f} | Net Profit: +${net_profit:.2f} ({net_edge_pct:.2f}% Net Edge)", flush=True)
...
async def subscription_manager(ws):
    """Monitors token_to_market and updates subscriptions whenever new tokens arrive."""
    subscribed_tokens = set()
    while True:
        try:
            current_tokens = list(token_to_market.keys())
            new_tokens = set(current_tokens) - subscribed_tokens

            if new_tokens and current_tokens:
                logger.info(f"[WS] Subscribing to {len(current_tokens)} asset IDs...")
                payload = {
                    "type": "market",
                    "assets_ids": current_tokens
                }
                await ws.send(json.dumps(payload))
                subscribed_tokens = set(current_tokens)

            await asyncio.sleep(10)
        except asyncio.CancelledError:
            break
        except Exception as e:
            logger.warning(f"Subscription manager error: {e}")
            await asyncio.sleep(5)


async def market_discovery_loop():
    """Periodically fetches active crypto markets every 60 seconds."""
    ...
    while True:
        ...
        if new_markets:
            ...
            logger.info(f"[DISCOVERY] Total monitored markets: {len(active_markets)}.")


async def websocket_listener():
    """Manages connection and event processing for Polymarket WebSocket stream."""
    global active_markets, token_to_market
    ...
    while True:
        try:
            async with websockets.connect(
                ...
                # Launch background task to manage subscriptions independently
                sub_task = asyncio.create_task(subscription_manager(ws))
                                    ...
                                    if asset_id in token_to_market:
                                        ...
                                        if best_ask:
                                            new_ask = [{
                                                ...
                                                "size": float(change.get("size", 100))
                                            }]
                                            if outcome_type == "UP":
                                                active_markets[market_id]["up_asks"] = new_ask
                                            else:
                                                active_markets[market_id]["down_asks"] = new_ask
                                            evaluate_arbitrage(market_id)
...
async def main():
    print("=" * 75, flush=True)
    print("   POLYMARKET WEBSOCKET BOT V6.2 (STDOUT & WS SUB FIX)", flush=True)
    print("=" * 75, flush=True)
    print(f" Initial Balance : ${INITIAL_BALANCE:.2f}", flush=True)
    print(f" Est. Gas Fee    : ${EST_GAS_FEE_USD:.2f} / trade", flush=True)
    print(f" Taker Fee Rate  : {CRYPTO_TAKER_FEE_RATE*100:.1f}%", flush=True)
    print(f" Minimum Gross   : {MIN_EDGE*100:.1f}%\n", flush=True)

    # 1. Perform initial discovery BEFORE opening WebSocket
    logger.info("Performing initial market discovery...")
    initial_markets, initial_tokens = fetch_crypto_markets()
    active_markets.update(initial_markets)
    token_to_market.update(initial_tokens)
    logger.info(f"Discovered {len(active_markets)} crypto markets on startup.")

    # 2. Run background discovery and WebSocket listener concurrently
